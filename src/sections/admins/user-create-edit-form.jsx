...
const onSubmit = handleSubmit(async (formData) => {
  try {
    ...
    const res = await axios({
      method: currentUser ? 'put' : 'post', // PUT if editing, POST if creating
      url: currentUser
        ? endpoints.users.update(currentUser.id)
        : endpoints.users.create,
      data: { ...formData, avatarUrl: null },
      headers: {
    ...
    if (res.data.success === true) {
      ...
      toast.error(currentUser ? 'Failed to update profile' : 'Failed to create user');
    }
  } catch (error) {
    console.error(error);
